...
const getChildren: GetChildren = async (
  ...
  if (SceneType === 'Group') {
    ...
    if (remoteResponseJson.status === 'OK') {
      return remoteResponseJson.result.map(mapIncludedSwitch(SceneStatus));
    }
    return false;
  }
  return false;
...
export class SwitchesController {
  ...
  async getSwitches(): Promise<SwitchesResponse> {
    const domoticzuri = this.configService.get<string>('DOMOTICZ_URI') || '';

    this.logger.verbose(`GET to /api/switches domoticzuri: ${domoticzuri}]`);
      ...
      try {
        ...
        if (remoteResponseJson.status === 'OK') {
          const switches = await Promise.all(
            (remoteResponseJson.result as DomoticzSwitch[]).map(mapSwitch(domoticzuri)),
          );
