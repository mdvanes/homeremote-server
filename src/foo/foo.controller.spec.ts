...
describe("Foo Controller", () => {
    ...
    it("logs a lot of stuff", async () => {
        ...
        expect(mocked(fooLoggerInstance.error).mock.calls[0][0]).toEqual(
            "Error: GET to /api/foo"
        );
        expect(mocked(fooLoggerInstance.warn).mock.calls[0][0]).toEqual(
            "Warning: GET to /api/foo"
        );
        expect(mocked(fooLoggerInstance.log).mock.calls[0][0]).toEqual(
            "Log: GET to /api/foo"
        );
        expect(mocked(fooLoggerInstance.debug).mock.calls[0][0]).toEqual(
            "Debug: GET to /api/foo"
        );
        expect(mocked(fooLoggerInstance.verbose).mock.calls[0][0]).toEqual(
            "Verbose: GET to /api/foo"
        );
    });
});
